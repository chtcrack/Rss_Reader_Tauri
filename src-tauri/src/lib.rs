...
use tauri::{Emitter, Manager, State, async_runtime::Mutex};
...
struct AppState {
    ...
    last_update_time: Mutex<chrono::DateTime<chrono::Utc>>,
}
...
/// 异步发送新文章通知（不阻塞主线程）
fn send_new_article_notification(
    app: &tauri::AppHandle,
    feed_name: &str,
    article_title: &str,
...
async fn init_ai_translator(app_state: State<'_, AppState>) -> Result<(), String> {
    ...
    // 创建带有默认AI平台的新翻译器实例
    let translator = AI_TRANSLATOR
        .get_translator()
        .await
        .with_default_platform(default_platform.clone());
    eprintln!(
        "[AI] 成功创建翻译器实例并设置默认AI平台: {:?}",
        default_platform
    );
    ...
    tokio::spawn(async move {
        ...
        if let Ok(articles) = result {
            ...
            if let Some(window) = app_clone.get_webview_window("main") {
                ...
                // 获取数据库锁并筛选需要翻译的文章
                {
                    ...
                    for article in articles {
                        if new_feed.translate_enabled {
                            // 检查文章是否需要翻译
                            let needs_translation = db_manager
                                .article_needs_translation(article.feed_id, &article.link)
                                .unwrap_or(true);
                ...
                println!(
                    "更新完成: {} 新增 {} 篇文章",
                    new_feed.name, new_article_count
                );
            }
...
async fn search_articles(
    ...
    limit: u32,
    feed_id: Option<i64>,
) -> Result<Vec<(Article, String)>, String> {
    let db_manager = app_state.db_manager.lock().await;
    db_manager
        .search_articles(query, limit, feed_id)
        .map_err(|e| {
...
async fn mark_article_as_read(
    ...
    let mut db_manager = app_state.db_manager.lock().await;
    db_manager
        .mark_article_as_read(articleId, isRead)
        .map_err(|e| {
...
async fn get_articles_by_feed(
    ...
    let db_manager = app_state.db_manager.lock().await;
    let mut articles = db_manager
        .get_articles_by_feed(feedId, limit, offset)
        .map_err(|e| {
...
async fn toggle_favorite(
    ...
    let mut db_manager = app_state.db_manager.lock().await;
    db_manager
        .toggle_favorite(articleId, isFavorite)
        .map_err(|e| {
...
// Tauri命令：获取收藏的文章
#[tauri::command(async)]
async fn get_favorite_articles(
    app_state: State<'_, AppState>,
    limit: u32,
    offset: u32,
) -> Result<Vec<Article>, String> {
    let db_manager = app_state.db_manager.lock().await;
    let mut articles = db_manager
        .get_favorite_articles(limit, offset)
        .map_err(|e| {
...
#[tauri::command(async)]
async fn get_favorite_articles_by_feed(
    app_state: State<'_, AppState>,
    feedId: i64,
    limit: u32,
    offset: u32,
    ...
    let db_manager = app_state.db_manager.lock().await;
    let mut articles = db_manager
        .get_favorite_articles_by_feed(feedId, limit, offset)
        .map_err(|e| {
            eprintln!(
                "Failed to get favorite articles by feed from database: {}",
                e
            );
...
#[tauri::command(async)]
async fn get_unread_articles_by_feed(
    app_state: State<'_, AppState>,
    feedId: i64,
    limit: u32,
    offset: u32,
    ...
    let db_manager = app_state.db_manager.lock().await;
    let mut articles = db_manager
        .get_unread_articles_by_feed(feedId, limit, offset)
        .map_err(|e| {
...
#[tauri::command(async, rename_all = "camelCase")]
async fn get_filtered_article_count(
    app_state: State<'_, AppState>,
    filter: &str,
    feed_id: Option<i64>,
    group_id: Option<i64>,
    ...
    let db_manager = app_state.db_manager.lock().await;
    db_manager
        .get_filtered_article_count(filter, feed_id, group_id, is_ungrouped)
        .map_err(|e| {
...
// Tauri命令：获取默认AI平台
#[tauri::command(async)]
async fn get_default_ai_platform(
    app_state: State<'_, AppState>,
) -> Result<Option<AIPlatform>, String> {
    let db_manager = app_state.db_manager.lock().await;
    db_manager.get_default_ai_platform().map_err(|e| {
        eprintln!("Failed to get default AI platform from database: {}", e);
        format!("Failed to get default AI platform: {}", e)
...
// Tauri命令：标记所有文章为已读
#[tauri::command(async, rename_all = "camelCase")]
async fn mark_all_articles_as_read(
    app_state: State<'_, AppState>,
    feed_id: Option<i64>,
) -> Result<(), String> {
    let mut db_manager = app_state.db_manager.lock().await;
...
// Tauri命令：设置默认AI平台
#[tauri::command(async)]
async fn set_default_ai_platform(
    app_state: State<'_, AppState>,
    platform_id: i64,
) -> Result<(), String> {
    // 更新数据库中的默认平台
    {
        let mut db_manager = app_state.db_manager.lock().await;
        db_manager
            .set_default_ai_platform(platform_id)
            .map_err(|e| {
    ...
    let _translator = AI_TRANSLATOR
        .get_translator()
        .await
        .with_default_platform(new_default_platform);
...
// Tauri命令：添加黑名单关键字
#[tauri::command(async)]
async fn add_blacklist_keyword(
    app_state: State<'_, AppState>,
    keyword: String,
) -> Result<i64, String> {
    let mut db_manager = app_state.db_manager.lock().await;
...
// Tauri命令：删除黑名单关键字
#[tauri::command(async)]
async fn delete_blacklist_keyword(
    app_state: State<'_, AppState>,
    keyword: String,
) -> Result<(), String> {
    let mut db_manager = app_state.db_manager.lock().await;
...
// Tauri命令：更新自动更新间隔
#[tauri::command(async)]
async fn update_update_interval(
    app_state: State<'_, AppState>,
    interval: u64,
) -> Result<(), String> {
    use std::fs::write;
...
async fn get_chat_sessions(app: tauri::AppHandle) -> Result<Vec<ChatSession>, String> {
    ...
    // 读取目录中的所有会话文件
    let entries =
        read_dir(chat_dir).map_err(|e| format!("Failed to read chat sessions directory: {}", e))?;
    ...
    for entry in entries {
        ...
        if let Some(extension) = path.extension() {
            if extension == "json" {
                // 读取文件内容
                let mut file =
                    File::open(&path).map_err(|e| format!("Failed to open session file: {}", e))?;
...
async fn save_chat_session(app: tauri::AppHandle, session: ChatSession) -> Result<(), String> {
    ...
    // 保存会话到文件
    let session_json = serde_json::to_string_pretty(&updated_session)
        .map_err(|e| format!("Failed to serialize session: {}", e))?;
...
// Tauri命令：获取更新剩余时间（秒）
#[tauri::command(async, rename_all = "camelCase")]
async fn get_update_remaining_time(
    app_state: State<'_, AppState>,
) -> Result<u64, String> {
    // 获取配置
    let config = {
    ...
    // 获取上次更新时间
    let last_update_time = { app_state.last_update_time.lock().await.clone() };
    let now = Utc::now();
    
    // 计算已过时间（秒）
    let elapsed = now.signed_duration_since(last_update_time).num_seconds();
    let elapsed = elapsed.max(0) as u64;
    
    // 计算剩余时间
    let remaining = if elapsed >= update_interval {
        0
    } else {
...
async fn ai_chat(
    ...
    // 调用AI翻译器的聊天函数
    match translator
        .chat_completion_stream(messages, max_tokens, temperature, tx)
        .await
    {
        Ok(_) => Ok(()),
        Err(e) => {
...
async fn update_all_feeds(
    ...
    // 2. 有失败记录但下次重试时间已到的源
    let now = Utc::now();
    let feeds_to_update: Vec<Feed> = all_feeds
        .into_iter()
        .filter(|feed| {
...
async fn auto_update_feeds_task(app: tauri::AppHandle) {
    ...
    loop {
        match process_auto_update(&app).await {
            Ok(update_interval) => {
                // 使用配置的更新间隔
...
// 处理单次自动更新逻辑的辅助函数
async fn process_auto_update(app: &tauri::AppHandle) -> Result<u64, Box<dyn std::error::Error + Send + Sync>> {
    ...
    let all_feeds = match {
        ...
        Err(e) => {
            ...
            return Ok(update_interval); // 继续循环，下次重试
        }
    };

    // 保存总源数量，避免后续使用被移动的all_feeds
    let total_feeds_count = all_feeds.len();
    println!("发现 {} 个RSS源", total_feeds_count);
    ...
    if !feeds_to_update.is_empty() {
        println!("需要更新 {} 个RSS源", feeds_to_update.len());
            ...
            match result {
                Ok((_, articles)) => {
                    ...
                    if let Err(e) = {
                        ...
                        eprintln!(
                            "Failed to update feed success status for {}: {}",
                            feed.name, e
                        );
                    }
                ...
                Err(e) => {
                    ...
                    if let Err(update_e) = {
                        ...
                        eprintln!(
                            "Failed to update feed failure status for {}: {}",
                            feed.name, update_e
                        );
                    }
                }
    ...
    // 更新上次更新时间
    {
        let mut last_update_time_guard = app_state.last_update_time.lock().await;
        *last_update_time_guard = Utc::now();
        println!("更新上次更新时间: {}", last_update_time_guard);
    }
...
async fn process_articles_sync(app_handle: tauri::AppHandle, feed: Feed, articles: Vec<Article>) {
    ...
    if let Some(platform) = default_platform {
        ...
        for (index, mut article) in articles.into_iter().enumerate() {
            ...
            let translation_result = translator
                .translate_rss_content(&article.title, &article.content, "zh-CN")
                .await;
                        ...
                        match {
                            ...
                            Ok(true) => {
                                send_new_article_notification(
                                    ...
                                    &feed_clone.name,
                                    &article_clone.title,
                                    article_clone.translated_title.as_deref(),
                                    feed_clone.notification_enabled,
                                );
...
fn get_config_path() -> std::path::PathBuf {
    ...
    match std::env::current_dir() {
        ...
        Err(e) => {
            eprintln!(
                "警告: 获取当前目录失败: {}, 将使用当前目录下的config.toml",
                e
            );
...
fn determine_db_path(config: &Config) -> String {
    ...
    let path_type = match &config.db {
        Some(db_config) => db_config.path_type.as_deref().unwrap_or("user"),
        None => {
            "user" // 默认值
        }
    };
    ...
    let custom_path = match &config.db {
        Some(db_config) => db_config.custom_path.clone(),
        None => None,
    };
...
pub fn run() {
    ...
    builder
        ...
        .manage(AppState {
            ...
            last_update_time: Mutex::new(Utc::now()),
        })
